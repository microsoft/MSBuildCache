...
/// </summary>
internal static class CompilerUtilities
{
    ...
    public static CommandLineArguments GetParsedCommandLineArguments(string language, string[] arguments, string projectFile)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(arguments), "Length must be greater than 0.");
        }
        ...
        {
            LanguageNames.CSharp => CSharpCommandLineParser.Default.Parse(arguments, projectDirectory, sdkDirectory),
            LanguageNames.VisualBasic => VisualBasicCommandLineParser.Default.Parse(arguments, projectDirectory, sdkDirectory),
    ...
    public static string[] GetEmbeddedResourceFilePaths(IEnumerable<string> embeddedResourceArgs, string? baseDirectory)
    {
        ...
        {
            if (TryParseOption(embeddedResourceArg, out string argName, out string argValue))
            {
